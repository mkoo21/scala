...
abstract class TweetSet {
   ...
   *
   * EmptySet returns an empty tweet set; NonEmptySets return the accumulated left and right children.
   */
   ...
   *
   * If `this.contains(tweet)`, the current set is reurned.
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = { 
      if (p(this.elem)) left.filterAcc(p, right.filterAcc(p, acc.incl(this.elem)))
    ...
    def descendingByRetweet(): TweetList = {
      ...
      val remainder = this.remove(head)
      new Cons(head, remainder.descendingByRetweet)
